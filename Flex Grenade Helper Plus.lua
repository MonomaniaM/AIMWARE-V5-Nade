...
local ref = gui.Tab(gui.Reference("Misc"), "flex_nade", "Flex 投掷物助手");
local guiHelperSettingsBlock = gui.Groupbox(ref, "投掷物助手设置", 16, 16, 250, 250);
local guiHelperEnable = gui.Checkbox(guiHelperSettingsBlock , "nade_enable", "开启", 1);
local guiHelperDrawDistance = gui.Slider(guiHelperSettingsBlock, "nade_draw_dist", "点位显示距离", 350, 0, 1000);
local guiHelperPDrawDistance = gui.Slider(guiHelperSettingsBlock, "nade_pdraw_dist", "点位指向线显示距离", 20, 0, 100);
local guiHelperActivationDistance = gui.Slider(guiHelperSettingsBlock, "nade_activation_dist", "自动投掷有效距离", 60, 0, 300);
local guiHelperFov = gui.Slider(guiHelperSettingsBlock, "nade_fov", "自动投掷瞄准范围", 10, 0, 250);
local guiHelperTextColor = gui.ColorPicker(guiHelperSettingsBlock, "nade_text_color", "文字颜色", 171, 153, 242, 255);
local guiHelperCircleColor = gui.ColorPicker(guiHelperSettingsBlock, "nade_circle_color", "圆圈颜色", 171, 153, 242, 255);
local guiHelperLineColor = gui.ColorPicker(guiHelperSettingsBlock, "nade_line_color", "线条颜色", 171, 153, 242, 255);
local guiHelperBackColor = gui.ColorPicker(guiHelperSettingsBlock, "nade_back_color", "背景颜色", 0, 0, 0, 120);
local guiHelperMiscBlock = gui.Groupbox(ref, "其他设置", 16, 446, 250, 250);
local guiHelperFixGrenadePred = gui.Checkbox(guiHelperMiscBlock, "nade_fixpredict", "修正投掷物助手点位", 0);
...
local function DrawNadeInfo()
	...
	if localWeapon ~= 44 and localWeapon ~= 46 and localWeapon ~= 48 and localWeapon ~= 45 and localWeapon ~= 43 then
		return;
	end
	...
	for i = 1, #NadesData do
		...
		elseif nadeType == 4 then
			if localWeapon ~= 45 then 
				goto continue;
			end	
		elseif nadeType == 5 then
			if localWeapon ~= 43 then 
				goto continue;
			end	
...
local function MovmentNadeHelper(cmd)
	...
	if not startMoving then
		...
		elseif NadesData[nadeClosestIndex][4] == 2 then
			if localWeapon ~= 46 and localWeapon ~= 48 then 
				return;
			end
		elseif NadesData[nadeClosestIndex][4] == 4 then
			if localWeapon ~= 45 then 
				return;
			end
		elseif NadesData[nadeClosestIndex][4] == 5 then
			if localWeapon ~= 43 then 
				return;
			end
		end
